...
function CustomerBuyed() {
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [customerBuyed, setCustomerBuyed] = useState([]);
  const [textSearchCustomerBuyed, setTextSearchCustomerBuyed] = useState("");
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultSearchCustomerBuyed =
        await getSearchCustomerBuyed.getSearchCustomerBuyed(debounceValue);
      setCustomerBuyed(resultSearchCustomerBuyed.data);
    };
    fetchAPI();
  }, [debounceValue]);
  ...
  const totalPage = Math.ceil(customerBuyed.length / itemPerPage);
  const startIndex = (currentPage - 1) * itemPerPage;
  const endIndex = startIndex + itemPerPage;
  const currentItems = customerBuyed.slice(startIndex, endIndex);
