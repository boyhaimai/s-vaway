...
import * as getSearchOrderConfirmedService from "~/service/getSearchOrderConfirmed";
import * as getQuantityOrderConfirmedService from "~/service/getQuantityOrderConfirmed";
...
function Confirmed() {
  ...
  const [textSearchOrderConfirmed, setTextSearchOrderConfirmed] = useState("");
  const inputSearchRef = useRef();
  const debounceValue = useDebounce(textSearchOrderConfirmed, 800);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultOrderConfirmed =
        await getOrderConfirmedService.getOrderConfirmedService();
      setOrderConfirmed(resultOrderConfirmed.data);
    };
    fetchAPI();
  }, []);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultSearchOrderCofirmed =
        await getSearchOrderConfirmedService.getSearchOrderConfirmedService(
          debounceValue
        );
      setOrderConfirmed(resultSearchOrderCofirmed.data);
    };
    fetchAPI();
  }, [debounceValue]);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultQuantity =
        await getQuantityOrderConfirmedService.getQuantityOrderConfirmedService(
          quantity
        );
      setOrderConfirmed(resultQuantity.data);
    };
    fetchAPI();
  }, [quantity]);
  ...
  const handleChangeSearch = (event) => {
    ...
    if (!value.startsWith(" ")) {
      setTextSearchOrderConfirmed(value);
    }
  };

  const handleClearInputSeach = () => {
    setTextSearchOrderConfirmed("");
  ...
  const totalPage = Math.ceil(orderConfirmed.length / itemPerPage);
  const startIndex = (currentPage - 1) * itemPerPage;
  const endIndex = startIndex + itemPerPage;
  const currentItems = orderConfirmed.slice(startIndex, endIndex);
