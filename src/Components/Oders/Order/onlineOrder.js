...
import * as getOrderOnlineService from "~/service/getOrderOnlineService";
import * as getSearchOrderOnlineService from "~/service/getSearchOrderOnlineService";
import * as getQuatityOrderOnlineService from "~/service/getQuatityOrderOnlineService";
...
function OnlineOrder() {
  ...
  const [textSearchOnlineOrder, setTextSearchOnlineOrder] = useState("");
  const inputSearchRef = useRef();
  const debounceValue = useDebounce(textSearchOnlineOrder, 800);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultOnlineOrder =
        await getOrderOnlineService.getOrderOnlineService(quantity);
      setOnlineOrder(resultOnlineOrder.data);
    };
    fetchAPI();
  }, [quantity]);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultQuantity =
        await getQuatityOrderOnlineService.getQuatityOrderOnlineService(
          quantity
        );
      setOnlineOrder(resultQuantity.data);
    };
    fetchAPI();
  }, [quantity]);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultOnlineOrder =
        await getOrderOnlineService.getOrderOnlineService();
      setOnlineOrder(resultOnlineOrder.data);
    };
    fetchAPI();
  }, []);
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      const resultSearchOrderOnline =
        await getSearchOrderOnlineService.getSearchOrderOnlineService(
          debounceValue
        );
      setOnlineOrder(resultSearchOrderOnline.data);
    };
    fetchAPI();
  }, [debounceValue]);
  ...
  const handleChangeSearch = (e) => {
    ...
    if (!value.startsWith(" ")) {
      setTextSearchOnlineOrder(value);
    }
  };

  const handleClearInputSeach = () => {
    setTextSearchOnlineOrder("");
    inputSearchRef.current.focus();
  };

  const totalPage = Math.ceil(onlineOrder.length / itemPerPage);
  const startIndex = (currentPage - 1) * itemPerPage;
  const endIndex = startIndex + itemPerPage;
  const currentItems = onlineOrder.slice(startIndex, endIndex);
